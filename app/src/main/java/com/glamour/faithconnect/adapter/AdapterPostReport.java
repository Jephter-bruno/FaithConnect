...
public class AdapterPostReport extends RecyclerView.Adapter<AdapterPostReport.MyHolder>{

    final Context context;
    final List<ModelPost> modelPosts;

    public AdapterPostReport(Context context, List<ModelPost> modelPosts) {
    ...
    public void onBindViewHolder(@NonNull MyHolder holder, int position) {
        ...
        holder.itemView.setOnClickListener(v -> {
            ...
            popupMenu.getMenu().add(Menu.NONE,4,0, "Delete post");

            popupMenu.setOnMenuItemClickListener(item -> {
                int id = item.getItemId();

                if (id == 1) {
                    FirebaseDatabase.getInstance().getReference("warn").child("user").child(modelPosts.get(position).getId()).setValue(true);
                    //Notification
                    String timestamp = ""+System.currentTimeMillis();
                    HashMap<Object, String> hashMap = new HashMap<>();
                    hashMap.put("pId", modelPosts.get(position).getpId());
                    hashMap.put("timestamp", timestamp);
                    hashMap.put("pUid",  modelPosts.get(position).getId());
                    hashMap.put("notification", "You have got a warning by the admin");
                    hashMap.put("sUid", FirebaseAuth.getInstance().getCurrentUser().getUid());
                    FirebaseDatabase.getInstance().getReference("Users").child(modelPosts.get(position).getId()).child("Notifications").child(timestamp).setValue(hashMap);
                    FirebaseDatabase.getInstance().getReference("Users").child(modelPosts.get(position).getId()).child("Count").child(timestamp).setValue(true);
                    notify = true;
                    FirebaseDatabase.getInstance().getReference("Users").child(FirebaseAuth.getInstance().getCurrentUser().getUid()).addListenerForSingleValueEvent(new ValueEventListener() {
                        @Override
                        public void onDataChange(@NonNull DataSnapshot snapshot) {
                            ModelUser user = snapshot.getValue(ModelUser.class);
                            if (notify){
                                sendNotification(modelPosts.get(position).getId(), Objects.requireNonNull(user).getName(), "You have got a warning by the admin");
                            }
                            notify = false;
                        }
                ...
                if (id == 2) {
                    FirebaseDatabase.getInstance().getReference().child("ReportPost").child(modelPosts.get(position).getId()).getRef().removeValue();
