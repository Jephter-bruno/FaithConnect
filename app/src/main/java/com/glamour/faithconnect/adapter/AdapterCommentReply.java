...
import com.glamour.faithconnect.model.ModelCommentReply;
...
public class AdapterCommentReply extends RecyclerView.Adapter<AdapterCommentReply.MyHolder>{
    ...
    final List<ModelCommentReply> modelComments;
    NightMode nightMode;

    public AdapterCommentReply(Context context, List<ModelCommentReply> modelComments) {
       ...
       if (position>1 && (position+1) % 3 == 0) {
        ...
        morePop.setOnMenuItemClickListener(item -> {
           if (item.getItemId() == 4){
               ...
               FirebaseDatabase.getInstance().getReference("Reply").child(modelComments.get(position).getrId()).child(modelComments.get(position).getcId()).getRef().removeValue();
